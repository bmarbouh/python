...
class FantasyCardFactory(CardFactory):
    ...
    def create_artifact(self, name_or_power: str) -> Card:
        ...
        if name_or_power in data:
            c = data[name_or_power]
            player = ArtifactCard(
                c['name'], c['cost'], c['rarity'], c['durability'], c['effect']
            )
            return player
        ...
        for _ in range(size):
            ...
            if e == 'creature':
                deck['creature'].append(
                    self.create_creature(random.choice(creature_names))
                )
            elif e == 'spell':
                deck['spell'].append(
                    self.create_spell(random.choice(spell_names))
                )
            elif e == 'artifact':
                deck['artifact'].append(
                    self.create_artifact(random.choice(artifact_names))
                )
        return deck
