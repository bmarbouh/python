...
def acsess_data(data: list):
    data_gen = generate_data(data)
    count = 0
    ...
    for event in data_gen:
        if count <= 4:
            print(
                f"Event {count + 1}: Player {event['player']} "
...
def stream_analytics(data: list):
    data_gen = generate_data(data)
    total_event = 0
    high_level = 0
    treasure_event = 0
...
def demonstaration_gen(data: list):
    print("=== Generator Demonstration ===")
    start = True
    str = ""
    for n in fibinocci(10):
        if not start:
            str += f", {n}"
        else:
            str += f"{n}"
            start = False
    print(f"Fibonacci sequence (first 10): {str}")
    start = True
    str = ""
    for n in prime_gen(5):
        if not start:
            ...
            str += f"{n}"
            start = False
    print(f"Prime numbers (first 5): {str}")
